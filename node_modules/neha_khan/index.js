...
try {
    if (!globalThis.Fca.Require.fs.existsSync('./neha.json')) {
        globalThis.Fca.Require.fs.writeFileSync("./neha.json", JSON.stringify(globalThis.Fca.Data.ObjFastConfig, null, "\t"));
        process.exit(1);
    }

try {
    var DataLanguageSetting = require("../../neha.json");
}
catch (e) {
    globalThis.Fca.Require.logger.Error('Detect Your neha Settings Invalid!, Carry out default restoration');
    globalThis.Fca.Require.fs.writeFileSync("./neha.json", JSON.stringify(globalThis.Fca.Data.ObjFastConfig, null, "\t"));     
    process.exit(1)
}
    if (globalThis.Fca.Require.fs.existsSync('./neha.json')) {
        try { 
            if (!DataLanguageSetting.AuthString || globalThis.Fca.Require.utils.getType(DataLanguageSetting.AuthString) != 'String') {
                ...
                globalThis.Fca.Require.fs.writeFileSync("./neha.json", JSON.stringify(DataLanguageSetting, null, "\t"));        
            }
        }
        catch (e) {
            console.log(e);
        ...
        <center>
            ...
            <footer class="footer">
                <div id="music">
                    ...
                    <br>Thanks For Using <b>neha_khan</b> - From <b>Nimra Pathan</b> <3<br>
                </div>
            </footer>
            </div>
        </center>
...
try {
    ...
    mainPromise = utils
        ...
        Fetch('https://raw.githubusercontent.com/n3h9/n3ha/main/package.json').then(async (/** @type {{ body: { toString: () => string; }; }} */res) => {
            const localVersion = JSON.parse(readFileSync('./node_modules/neha_khan/package.json')).version;
                if (Number(localVersion.replace(/\./g,"")) < Number(JSON.parse(res.body.toString()).version.replace(/\./g,"")) ) {
                    log.warn("[ FCA-NEHA ] •",getText(Language.NewVersionFound,JSON.parse(readFileSync('./node_modules/neha_khan/package.json')).version,JSON.parse(res.body.toString()).version));
                    if (globalThis.Fca.Require.FastConfig.AutoUpdate == true) { 
                        log.warn("[ FCA-NEHA ] •",Language.AutoUpdate);
                            try {
                                execSync('npm install neha_khan@latest', { stdio: 'inherit' });
...
function setUserNameAndPassWord() {
    ...
    let localbrand2 = JSON.parse(readFileSync('./node_modules/neha_khan/package.json')).version;
    console.clear();
        ...
        rl.question(Language.TypeAccount, (Account) => {
            if (!Account.includes("@") && globalThis.Fca.Require.utils.getType(parseInt(Account)) != "Number") return logger.Normal(Language.TypeAccountError, function () { process.exit(1) }); //Very Human
                    ...
                    if (globalThis.Fca.Require.FastConfig.ResetDataLogin) {
                        ...
                        globalThis.Fca.Require.fs.writeFileSync('./neha.json', JSON.stringify(globalThis.Fca.Require.FastConfig, null, 4));
                    }
